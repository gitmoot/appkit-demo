...
2. `__DATA_ROOT__/rationale.md`, one line for every extracted marketing key in the form `key: relative/source/file: concise extraction reason`.

Use deterministic repository evidence only. Do not use the network. Do not infer a company, email, color, tagline, or feature that is not written in a file. Do not edit the target repository. Prefer omission because downstream defaults fill gaps. Use one YAML `key: value` per line, with string scalars only and no nested objects, lists, aliases, tags, block scalars, or inline comments. Return the normal Gitmoot implemented result only after both files have been written.
