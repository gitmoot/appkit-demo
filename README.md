...
The personal DAG forks after derivation: `capture -> compose-real` builds the visual branch while `content` independently builds copy, legal sources, and icons. `landing` joins `content` plus `compose-real`, assembles the complete artifact tree, and `kit` depends only on landing. Compose-real persists verified marketing frames and transparent devices under `framed/`; content persists its verified branch under `content/`. Landing and kit consume and verify both manifest-backed handoffs, so the workflow remains correct with isolated stage worktrees and with concurrent stages sharing a checkout. The capture ladder first looks for qualifying exported store/marketing screenshots, then tries a Flutter or already-built web surface with stable-state DOM/canvas interactions, then falls back to deterministic synthetic screens. Real capture reports and generated README provenance state the honest real and synthetic-padding counts; real ladders never pad missing views. Vendored nested repositories, identified by their own `.git` file or directory, are never searched for exports. Web capture is browser fidelity, not an iOS or Android simulator.
