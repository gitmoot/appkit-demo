...
Set `APPKIT_PRO_DATA_DIR` to an absolute private directory to isolate the side channel from the default `/root/appkit-pro-data`. Set it before writing `target`, generating the spec, and starting/running the pipeline; the generator substitutes the same directory into the produce write grant and derive prompt. `order.yaml` carries the selected target path and is rejected if the current `target` file points elsewhere. Changes within the same target repository after derivation are not detected in v1, so regenerate after meaningful source changes.
