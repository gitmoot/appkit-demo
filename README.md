...
`appkit-pro` is a separate personal workflow for turning a trusted local app repository into the same launch kit with repo-grounded copy and real screenshots where possible. It is deliberately not service-safe or exposable: its generated spec contains produce stages with static reads or `/root/appkit-pro-data` writes, and derive also declares network permission. Those non-shell stages and grants make Gitmoot's expose safety check reject the pipeline.

The personal DAG forks after derivation: the shell `capture -> compose-real` chain builds the visual branch while an isolated `content` produce job writes grounded copy and legal sources. The registered `appkit-deriver` seat pins this job to `gpt-5.6-sol` with low effort. `landing` joins `content` plus `compose-real`, assembles the complete artifact tree, and `kit` depends only on landing. Compose-real persists verified marketing frames and transparent devices under `framed/`; content writes observed text under `content-agent/`. Landing validates every expected text file and regenerates only missing or malformed files through the deterministic templates. It also generates deterministic icons and records each content file as `agent` or `deterministic-fallback` in the README and manifest. Agent text is treated as observed rather than cross-stage deterministic; screens, fallback text, icons, and other deterministic outputs remain digest-verified. The capture ladder first looks for qualifying exported store/marketing screenshots, then tries a Flutter or already-built web surface with stable-state DOM/canvas interactions, then falls back to deterministic synthetic screens. Real capture reports and generated README provenance state the honest real and synthetic-padding counts; real ladders never pad missing views. Vendored nested repositories, identified by their own `.git` file or directory, are never searched for exports. Web capture is browser fidelity, not an iOS or Android simulator.
...
The current development CLI upserts a pipeline definition with plain `pipeline add`; if it reports that `--force` is unknown, rerun that line without `--force`. The generator records both the SHA-256 of `templates/appkit-pro.yaml.tmpl` and the resolved target path in `$APPKIT_PRO_DATA_DIR/spec-stamp`, while the generated spec also carries the template digest. Capture fails with `stale_spec` if the checked-out template changes, or `switch_target_requires_regen` with the regeneration and re-add remedy if `target` changes behind the installed spec. Legacy one-line digest stamps remain accepted and use the existing `stale_order` backstop. On a target change through the documented generator path, the generator safely clears the old order, rationale, capture state, and downstream handoffs before rendering the new spec, forcing a fresh derivation; a same-target rerun preserves that cache. `/root/appkit-pro-data` is the default personal-trust side channel: `target` is the caller's input, derive writes `order.yaml` and `rationale.md`, capture writes `screens/` plus `capture-report.json`, compose-real freshly replaces `framed/`, and the content produce job freshly replaces `content-agent/`. Landing validates those sources and assembles the complete `out/`; the Pro kit verifies the deterministic subset, packages every selected artifact, then freshly replaces `/root/appkit-pro-data/kit/`. The assembled ZIP is `/root/appkit-pro-data/kit/launch-kit.zip` and is overwritten on every successful run. It also copies the capture report into the manifest and ZIP, records screenshot plus per-file content provenance, and carries the synthetic fallback warning in its manifest.

`appkit-notify-pro` is a separate internal trigger pipeline defined by `notify-pro.yaml`. It reads Telegram credentials from `/root/.config/gitmoot/appkit-notify.env` and sends the persistent Pro ZIP after a successful `appkit-pro` run, or a text-only fallback when the ZIP is unavailable. Its `env_keys` and network access deliberately make it unexposable. `APPKIT_PRO_DATA_DIR` moves the notification source together with the rest of the personal side channel.
